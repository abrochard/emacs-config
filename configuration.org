...
** Setup binding to browse remote repo from magit
Can also be combied with =browse-at-remote= package
#+begin_src emacs-lisp
  (with-eval-after-load 'magit
    (define-key magit-status-mode-map (kbd "@") #'forge-browse-remote))
#+end_src
...
** Copy filename with path relative to project
#+begin_src emacs-lisp
  (defun copy-filename-for-project ()
    (interactive)
    (let* ((repo-path (magit-toplevel))
           (path (file-relative-name (buffer-file-name) repo-path)))
      (kill-new path)
      (message (format "Copied '%s' to the clipboard." path))))
#+end_src
...
** Run the next org-babel code block with no confirmations

#+begin_src emacs-lisp
(defun org-execute-next-src-block ()
  "Execute the next Org Babel src block in the current buffer.
[[elisp:(org-execute-next-src-block)][▶ Run]]
"
  (interactive)
...
** Split window vertically instead of horizontally
Or the other way around, I can never remember
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (transient-define-prefix go-test ()
    ...
    ["Arguments"
     ...
     ("-i" "Ignore cached results" "-count=1")]
    ["Actions"
...
* Makefile
Some utility functions to deal with large makefiles
#+begin_src emacs-lisp
  (defun makefile-jump-to-target ()
    (interactive)
  ...
  (defun my-makefile-mode-hook ()
    "Custom key bindings for makefile mode."
    (local-set-key (kbd "M-.") 'makefile-jump-to-target))
