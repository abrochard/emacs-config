...
#+begin_src emacs-lisp
  ...
  (defun go-test-repo ()
    ...
    (let* ((repo-path (magit-toplevel))
           ...
           (buffer-name (format "*go-test-%s*" repo-name))
  ...
  (defun go-build-repo ()
    ...
    (let* ((repo-path (magit-toplevel))
           (repo-name (file-name-nondirectory (directory-file-name repo-path)))
           (buffer-name (format "*go-build-%s*" repo-name))
  ...
  (defun go-run-repo ()
    ...
    (let* ((repo-path (magit-toplevel))
           (repo-name (file-name-nondirectory (directory-file-name repo-path)))
           (buffer-name (format "*go-run-%s*" repo-name))
           (default-directory (format "%s/cmd/%s" repo-path repo-name)))
      (pop-to-buffer buffer-name)
      (delete-region (point-min) (point-max))
...
#+begin_src emacs-lisp
  (defun go-mod-update ()
    ...
    (let ((repo (match-string 0)))
      (async-shell-command (format "go get -u %s" repo) "*go mod update*")))
#+end_src
