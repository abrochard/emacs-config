...
#+begin_src emacs-lisp
  ...
  (defun go-test-pkg ()
    ...
    (interactive)
    (let* ((pkg-name (file-name-base (directory-file-name default-directory)))
           (buffer-name (format "*go-test-pkg: %s*" pkg-name)))
      ;; (kill-buffer buffer-name)
      (pop-to-buffer buffer-name)
      (delete-region (point-min) (point-max))
      (start-process buffer-name buffer-name "go" "test" ".")))
  ...
  (defun go-test-repo ()
    ...
    (let* ((repo-path (magit-toplevel))
           ...
           (buffer-name (format "*go-test-repo: %s*" repo-name))
           (default-directory repo-path))
      (pop-to-buffer buffer-name)
      (delete-region (point-min) (point-max))
      (start-process buffer-name buffer-name "go" "test" "./..." "-cover")))
  ...
  (defun go-build-repo ()
    ...
    (let* ((repo-path (magit-toplevel))
           (repo-name (file-name-nondirectory (directory-file-name repo-path)))
           (buffer-name (format "*go-build-repo: %s*" repo-name))
           (default-directory (format "%s/cmd/%s" repo-path repo-name)))
      (pop-to-buffer buffer-name)
      (delete-region (point-min) (point-max))
      (start-process buffer-name buffer-name "go" "build")))
  ...
  (defun go-run-repo ()
    ...
    (let* ((repo-path (magit-toplevel))
           (repo-name (file-name-nondirectory (directory-file-name repo-path)))
           (buffer-name (format "*go-run-repo: %s*" repo-name))
           (default-directory (format "%s/cmd/%s" repo-path repo-name)))
      (pop-to-buffer buffer-name)
      (delete-region (point-min) (point-max))
      (start-process buffer-name buffer-name "go" "run" ".")))
