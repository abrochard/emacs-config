...
#+BEGIN_SRC emacs-lisp
  (require 'f)
  (let ((minions-directory (concat (file-name-as-directory config-load-path) "extra/minions/")))
    (if (not (f-exists? minions-directory))
	(progn
	  (shell-command
	   (mapconcat 'identity (list "git" "clone" "https://github.com/tarsius/minions.git" minions-directory) " "))
	  (kill-buffer "*Shell Command Output*")))
    (load-file (concat minions-directory "minions.el"))
    (minions-mode 1))
