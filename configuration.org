...
#+BEGIN_SRC  emacs-lisp
  ...
  (defun load-light-theme ()
    ...
    (custom-set-faces
     '(mode-line ((t (:background "DeepSkyBlue2" :foreground "white"))))
     '(mode-line-buffer-id ((t (:foreground "white"))))
     '(mode-line-inactive ((t (:background "dark grey"))))))
...
#+begin_src emacs-lisp
  ...
  (defun go-test-pkg ()
    ...
    (interactive)
    (compile "go test -cover -race -timeout 60s ."))
  ...
  (defun go-test-repo ()
    ...
    (let* ((repo-path (magit-toplevel))
      ...
      (compile "go test -cover -race ./...")))

  (defun go-coverage-here ()
    "Show go coverage of current file."
    (interactive)
    (shell-command "go test . -coverprofile=cover.out -race")
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key python-mode-map (kbd "C-c C-v") 'helm-projectile-ag)
#+END_SRC
